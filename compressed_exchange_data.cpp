...
namespace
{
	...
	// учитывает ли delta number - отсутствие 0 deltы??
	// учитывает ли nnds данные nnd??
	// учитывает ли f_delta повторение price c предыд. цен
	...
	// сколько позиций .price с начала не изменилось
	const _cdf3 nnse0(1, { 14, 12, 26, 16, 34, 114, 104, 146, 152, 200, 136, 338, 440, 472, 344, 312, 376, 504, 466, 3 });

	// сколько позиций .number не изменилось (с произвольной позиции)
	...
	// !! delta number, при 66 < number <= 200
	...
	// !! delta number, при 200 < number <= 318
	...
	// сколько позиций .price не изменилось (с произвольной позиции)
	...
	i64 calc_delta_del_add(const _offer* c, const std::vector<_offer>& v)
	{
		...
		if (c[1].price > c[0].price)
		{
			...
			{
				for (i64 i = 1; i < size_offer; i++) if (c[i].price >= v[0].price) return i;
				return size_offer;
			}
	...
	{
		i64 k = std::min(size_offer, (i64)v.size());
...
bool _compressed_exchange_data::read12(_offer* v1, std::vector<_offer>& v0)
{
	...
	if (izm > 0)
	{
		...
		n = izm;
		i64 n2 = nnsegg[std::min((i64)v0.size(), size_offer) - n].decoding(data);
		if (n2 == 0)
			v0.erase(v0.begin() + n);
		else
		{
...
bool _compressed_exchange_data::add12(const _offer* v1, std::vector<_offer>& v0, i64 izm)
{
	...
	if (izm < 0)
	{
		...
		if (!nnsegg[std::min((i64)v0.size(), size_offer)].coding(n2, data)) return false;
		if (n2 == 0)
		{
	...
	if (izm > 0)
	{
		...
		if (!nnsegg[std::min((i64)v0.size(), size_offer) - n].coding(n2, data)) return false;
		if (n2 == 0)
			v0.erase(v0.begin() + n);
		else
		{
